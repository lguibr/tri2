...
class SimpleStatsRecorder(StatsRecorderBase):
    ...
    def __init__(
        ...
        self.summary_avg_window = (
            StatsConfig.STATS_AVG_WINDOW[0] if StatsConfig.STATS_AVG_WINDOW else 100
        )
    ...
    def record_episode(
        ...
        game_state_for_best: Optional[
            "GameState"
        ] = None,  # This arg is also less relevant
    ):
    ...
    def log_summary(self, global_step: int):
        ...
        if not summary or "error" in summary:
            logger.error(
                f"Could not log summary, failed to fetch data: {summary.get('error', 'Unknown error')}"
            )
            return
        ...
        if new_best_score > self._last_best_score:
            self._log_new_best(
                ...
                summary.get("best_game_score_step", 0),
                is_loss=False,
            )
        ...
        if new_best_vloss < self._last_best_vloss:
            self._log_new_best(
                "V.Loss",
                new_best_vloss,
                self._last_best_vloss,
                summary.get("best_value_loss_step", 0),
                is_loss=True,
            )
        ...
        if new_best_ploss < self._last_best_ploss:
            self._log_new_best(
                "P.Loss",
                new_best_ploss,
                self._last_best_ploss,
                summary.get("best_policy_loss_step", 0),
                is_loss=True,
            )
        ...
        if new_best_mcts_time < self._last_best_mcts_time:
            self._log_new_best(
                "MCTS Sim Time",
                new_best_mcts_time,
                self._last_best_mcts_time,
                summary.get("best_mcts_sim_time_step", 0),
                is_loss=True,
                is_time=True,
            )
        ...
        best_score_str = (
            f"{new_best_score:.0f}" if new_best_score > -float("inf") else "N/A"
        )
        avg_win = summary.get("summary_avg_window_size", self.summary_avg_window)
        buf_size = summary.get("buffer_size", 0)
        min_buf = summary.get(
            "min_buffer_size", self.train_config.MIN_BUFFER_SIZE_TO_TRAIN
        )
        phase = "Buffering" if buf_size < min_buf and global_step == 0 else "Training"
        nodes_sec = summary.get("nodes_per_second_avg", 0.0)  # Get new metric
        ...
        log_items = [
            ...
            f"Nodes/s(Avg{avg_win}): {nodes_sec/1e3:<6.1f}k",  # Display new metric
        ...
        # Calculate ETA based on nodes/sec if available and positive
        if training_target_step > 0 and nodes_sec > 0 and phase == "Training":
            steps_remaining = training_target_step - global_step
            if steps_remaining > 0:
                # Estimate ETA based on training steps / sec (if available) or fallback
                steps_sec = summary.get("steps_per_second_avg", 0.0)
                if steps_sec > 0:
                    eta_seconds = steps_remaining / steps_sec
                    eta_str = format_eta(eta_seconds)
                    log_items.append(f"ETA: {eta_str}")
                # Could add an ETA based on nodes/sec but it's less direct for training steps
        elif training_target_step > 0 and phase == "Buffering":
            log_items.append("ETA: Buffering")
