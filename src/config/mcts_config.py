...
class MCTSConfig(BaseModel):
    ...
    # CHANGE: Increased for serious run (AlphaZero used 800 for Go/Chess)
    num_simulations: int = Field(800, ge=1)
    puct_coefficient: float = Field(1.0, gt=0)
    temperature_initial: float = Field(1.0, ge=0)
    temperature_final: float = Field(0.01, ge=0)
    temperature_anneal_steps: int = Field(10_000, ge=0)  # Anneal relatively quickly
