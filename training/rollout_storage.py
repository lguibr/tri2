...
class RolloutStorage:
    ...
    def __init__(
        ...
        self.device = device  # Target device (e.g., GPU)
        self.pin_memory = (
            self.device.type == "cuda"
        )  # Enable pinning only if using CUDA
        ...
        # Initialize storage tensors on CPU first, then pin if using CUDA
        # (+1 for next_obs/next_value/next_done)
        self.obs_grid = torch.zeros(
            num_steps + 1, num_envs, grid_c, grid_h, grid_w, device="cpu"
        )
        self.obs_shapes = torch.zeros(
            num_steps + 1, num_envs, shape_feat_dim, device="cpu"
        )
        self.obs_availability = torch.zeros(
            num_steps + 1, num_envs, shape_availability_dim, device="cpu"
        )
        self.obs_explicit_features = torch.zeros(
            num_steps + 1, num_envs, explicit_features_dim, device="cpu"
        )
        self.actions = torch.zeros(num_steps, num_envs, 1, device="cpu").long()
        self.log_probs = torch.zeros(num_steps, num_envs, 1, device="cpu")
        self.rewards = torch.zeros(num_steps, num_envs, 1, device="cpu")
        self.dones = torch.zeros(num_steps + 1, num_envs, 1, device="cpu")
        self.values = torch.zeros(num_steps + 1, num_envs, 1, device="cpu")
        self.returns = torch.zeros(num_steps, num_envs, 1, device="cpu")  # GAE returns

        # RNN Specific Data (also on CPU initially)
    ...
    def to(self, device: torch.device):
        """
        Moves storage tensors to the specified device.
        Note: This is generally NOT needed if using pinned memory correctly.
              The data should be moved just before the update loop in the Trainer.
              Keeping this method for potential debugging or alternative workflows.
        """
        ...
        self.device = device
        self.pin_memory = False  # Memory is no longer pinned if moved
    ...
    def insert(
        ...
        Assumes input tensors are on CPU and copies them to the (potentially pinned) storage.
        """
        ...
        # Ensure input tensors are on CPU before copying to pinned memory
        obs_grid_cpu = obs_grid.cpu()
        obs_shapes_cpu = obs_shapes.cpu()
        obs_availability_cpu = obs_availability.cpu()
        obs_explicit_features_cpu = obs_explicit_features.cpu()
        ...
        self.obs_grid[current_step_index].copy_(obs_grid_cpu)
        self.obs_shapes[current_step_index].copy_(obs_shapes_cpu)
        self.obs_availability[current_step_index].copy_(obs_availability_cpu)
        self.obs_explicit_features[current_step_index].copy_(obs_explicit_features_cpu)
        self.actions[current_step_index].copy_(action_cpu)
        self.log_probs[current_step_index].copy_(log_prob_cpu)
        self.values[current_step_index].copy_(value_cpu)
        self.rewards[current_step_index].copy_(reward_cpu)
        self.dones[current_step_index].copy_(done_cpu)

        if self.rnn_config.USE_RNN and lstm_state is not None:
            lstm_state_cpu = (lstm_state[0].cpu(), lstm_state[1].cpu())
            if self.hidden_states is not None and self.cell_states is not None:
                self.hidden_states[current_step_index].copy_(lstm_state_cpu[0])
                self.cell_states[current_step_index].copy_(lstm_state_cpu[1])

        # Store the *next* observation/done state at step+1 index
        next_step_index = current_step_index + 1
        self.obs_grid[next_step_index].copy_(obs_grid_cpu)
        self.obs_shapes[next_step_index].copy_(obs_shapes_cpu)
        self.obs_availability[next_step_index].copy_(obs_availability_cpu)
        self.obs_explicit_features[next_step_index].copy_(obs_explicit_features_cpu)
        self.dones[next_step_index].copy_(done_cpu)
        if self.rnn_config.USE_RNN and lstm_state is not None:
            if self.hidden_states is not None:
                self.hidden_states[next_step_index].copy_(lstm_state_cpu[0])
            if self.cell_states is not None:
                self.cell_states[next_step_index].copy_(lstm_state_cpu[1])
    ...
    def compute_returns_and_advantages(
        ...
        Assumes next_value and final_dones are on the agent's device.
        Performs calculations on CPU using the (potentially pinned) storage tensors.
        """
        if self.step != self.num_steps:
            print(
        ...
        self.values[last_step_index].copy_(next_value_cpu)
        # Store the done state *after* the last step in the rollout
        self.dones[last_step_index].copy_(final_dones_cpu)
    ...
    def get_data_for_update(self) -> Dict[str, Any]:
        ...
        The Trainer is responsible for moving this data to the agent's device.
        """
        effective_num_steps = self.step
