...
class MainApp:
    ...
    def _request_cleanup(self):
        ...
        # *** Set the flag to activate the overlay ***
    ...
    def _cancel_cleanup(self):
        ...
        # Set flag FIRST to stop overlay rendering immediately
        self.cleanup_confirmation_active = False
    ...
    def _confirm_cleanup(self):
        ...
        try:
            ...
            print(f"FATAL ERROR during _confirm_cleanup -> _cleanup_data call: {e}")
            traceback.print_exc()
            ...
            if event.key == pygame.K_LEFT:
                ...
                if action_index is not None:
                    ...
                    if self.buffer is not None:
                        ...
                        print("Warning: Replay buffer not available.")
                    action_taken = True
                else:
    ...
    def _cleanup_data(self):
        ...
        if self.renderer:
            try:  # Add try-except for rendering during cleanup
                self.renderer.render_all(
                    ...
                    num_envs=0,
                    env_config=self.env_config,
                    cleanup_confirmation_active=False,  # Explicitly false
                    cleanup_message="",
                    last_cleanup_message_time=0,
                    tensorboard_log_dir=None,
                    plot_data={},
        ...
        try:
            # *** Re-init RL components (can fail) ***
            self._initialize_rl_components()
        ...
        # --- Update logic specific to Playing state ---
            ...
            if self.demo_env:
                if hasattr(self.demo_env, "_update_timers") and callable(
                    ...
                    print("Warning: demo_env missing _update_timers method.")
            self.status = "Playing Demo"
