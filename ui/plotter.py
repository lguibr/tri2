...
class Plotter:
    """Handles creation and caching of the multi-plot Matplotlib surface, maintaining its own data cache."""

    def __init__(self):
        self.plot_surface_cache: Optional[pygame.Surface] = None
        self.last_plot_update_time: float = 0.0
        self.plot_update_interval: float = 2.0
        ...
        # --- Internal Data Cache ---
        # Stores the history received over time, respecting PLOT_DATA_WINDOW
        self.plot_data_cache: Dict[str, Deque] = self._initialize_data_cache()
        # --------------------------
    ...
    def _initialize_data_cache(self) -> Dict[str, Deque]:
        """Initializes empty deques for caching plot data."""
        cache = {}
        # Define all keys expected to be plotted
        plot_keys = [
            "game_scores",
            "episode_outcomes",
            "episode_lengths",
            ...
            "mcts_nodes_explored",
            "mcts_avg_depths",
            # Add any other keys you might plot
        ]
        for key in plot_keys:
            cache[key] = deque(maxlen=self.plot_data_window)
        return cache

    def _update_internal_cache(self, new_plot_data: Dict[str, List]):
        """Updates the internal data cache with new data received.
        Assumes new_plot_data contains lists as received from the aggregator.
        """
        for key, new_data_list in new_plot_data.items():
            if key in self.plot_data_cache:
                # Efficiently update deque: extend and let maxlen handle truncation
                # This assumes the new_data_list represents the *entire* current history
                # up to PLOT_DATA_WINDOW from the aggregator.
                # If the aggregator only sends *new* points, logic needs adjustment.
                # For now, assume it sends the current window state.
                self.plot_data_cache[key] = deque(
                    new_data_list, maxlen=self.plot_data_window
                )
            # else: # Optional: Log if unexpected keys are received
            #     logger.warning(f"[Plotter] Received unexpected plot data key: {key}")
    ...
    def _init_colors(self) -> Dict[str, Tuple[float, float, float]]:
        ...
        return {
            ...
            "nodes_per_second": normalize_color_for_matplotlib(
                VisConfig.GOOGLE_COLORS[0]
            ),
            # Placeholder Color
    ...
    def _init_figure(self, target_width: int, target_height: int):
        ...
        try:
            ...
            self.fig.subplots_adjust(
                hspace=0.4, wspace=0.3, left=0.08, right=0.97, bottom=0.06, top=0.96
            )
    ...
    def _get_data_hash(self) -> int:  # Use internal cache for hash
        """Generates a simple hash based on cached data lengths and last elements."""
        hash_val = 0
        keys_to_hash = list(self.plot_data_cache.keys())  # Hash all cached keys
        for key in sorted(keys_to_hash):
            dq = self.plot_data_cache.get(key)
    ...
    def _update_plot_data(self):  # No longer takes plot_data argument
        """Updates the data on the existing Matplotlib axes using the internal cache."""
        ...
        try:
            ...
            plot_defs = [
                ...
                ("placeholder", "Future Plot", self.colors["placeholder"], False),
            ]

            # Use internal cache for plotting
            data_lists = {
                key: list(self.plot_data_cache.get(key, deque()))
                for key, _, _, _ in plot_defs
            }
    ...
    def get_cached_or_updated_plot(
        self,
        new_plot_data: Dict[str, List],  # Expects list from aggregator
        target_width: int,
        target_height: int,
    ) -> Optional[pygame.Surface]:
        ...
        # --- Update internal cache first ---
        self._update_internal_cache(new_plot_data)
        # -----------------------------------

        has_data = any(d for d in self.plot_data_cache.values())  # Check cache
        ...
        current_data_hash = self._get_data_hash()  # Use internal cache for hash
        ...
        try:
            if needs_reinit:
                ...
                if self.fig:
                    if self._update_plot_data():  # Use internal cache
