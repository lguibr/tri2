...
if __name__ == "__main__":
    ...
    try:
        ...
        mlflow_tracking_uri = persist_config.MLFLOW_TRACKING_URI
        os.makedirs(
            os.path.dirname(mlflow_tracking_uri.replace("file:", "")), exist_ok=True
        )
        mlflow.set_tracking_uri(mlflow_tracking_uri)
        logger.info(f"Set MLflow tracking URI to: {mlflow_tracking_uri}")
...
class MCTSConfig(BaseModel):
    ...
    num_simulations: int = Field(80, ge=1) # Reduced simulations significantly
    puct_coefficient: float = Field(1.0, gt=0)
    temperature_initial: float = Field(1.0, ge=0)
    temperature_final: float = Field(0.01, ge=0)
    temperature_anneal_steps: int = Field(5_000, ge=0) # Reduced annealing steps slightly
...
class ModelConfig(BaseModel):
    ...
    NUM_RESIDUAL_BLOCKS: int = Field(3, ge=0)
    ...
    TRANSFORMER_DIM: int = Field(128, gt=0) # Dimension of transformer embeddings/output
    TRANSFORMER_HEADS: int = Field(4, gt=0) # Number of attention heads
    TRANSFORMER_LAYERS: int = Field(2, ge=0) # Number of transformer encoder layers
    TRANSFORMER_FC_DIM: int = Field(256, gt=0) # Dimension of the feed-forward layer inside transformer
    ...
    def check_transformer_config(self) -> 'ModelConfig':
        if self.USE_TRANSFORMER:
            if self.TRANSFORMER_LAYERS <= 0:
                raise ValueError("TRANSFORMER_LAYERS must be positive if USE_TRANSFORMER is True.")
            if self.TRANSFORMER_DIM <= 0:
                raise ValueError("TRANSFORMER_DIM must be positive if USE_TRANSFORMER is True.")
            if self.TRANSFORMER_HEADS <= 0:
                raise ValueError("TRANSFORMER_HEADS must be positive if USE_TRANSFORMER is True.")
            ...
            if self.TRANSFORMER_FC_DIM <= 0:
                raise ValueError("TRANSFORMER_FC_DIM must be positive if USE_TRANSFORMER is True.")
        return self

    @model_validator(mode='after')
    def check_transformer_dim_consistency(self) -> 'ModelConfig':
        if self.USE_TRANSFORMER and self.CONV_FILTERS:
            # Check if the output channels of the CNN/ResNet match the transformer input dim
            cnn_output_channels = self.RESIDUAL_BLOCK_FILTERS if self.NUM_RESIDUAL_BLOCKS > 0 else self.CONV_FILTERS[-1]
            if cnn_output_channels != self.TRANSFORMER_DIM:
                ...
                print(f"Warning: CNN output channels ({cnn_output_channels}) do not match TRANSFORMER_DIM ({self.TRANSFORMER_DIM}). Ensure the model handles this (e.g., with a projection layer).")
        return self
...
File: src\config\train_config.py
import time
...
File: src\environment\grid\triangle.py
...
File: src\environment\logic\__init__.py
...
class TrainingOrchestrator:
    ...
    def _initialize_workers(self):
        ...
        for i in range(self.train_config.NUM_SELF_PLAY_WORKERS):
            ...
            worker = SelfPlayWorker.options(num_cpus=1).remote(
                ...
                worker_device_str=self.train_config.WORKER_DEVICE, # Pass configured device
    ...
    def _update_worker_networks(self):
        ...
        # Use ray.put for efficient transfer of potentially large weights
        ...
        try:
            # Wait for acknowledgements with a timeout
            ray.get(update_tasks, timeout=15.0)
            logger.debug("Worker networks updated.")
        except ray.exceptions.RayActorError as e:
            logger.error(f"A worker actor failed during weight update: {e}", exc_info=True)
            # Potentially mark the failed worker and handle it (e.g., restart)
        except ray.exceptions.GetTimeoutError:
            logger.error("Timeout waiting for workers to update weights.")
        except Exception as e:
            logger.error(f"Unexpected error updating worker networks: {e}", exc_info=True)
                ...
                if ready_refs:
                    for ref in ready_refs:
                        ...
                        try:
                            ...
                            logger.error(f"Worker {worker_idx} failed: {e}", exc_info=True)
                            ...
                            if worker_idx < len(self.workers): self.workers[worker_idx] = None # Mark worker as failed
                            continue
                        # Relaunch task only if worker is still valid
